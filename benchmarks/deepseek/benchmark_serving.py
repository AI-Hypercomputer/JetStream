...
""".format(
  ...
  if method == 'HELM':
    for _, row in example_data.iterrows():
      examples += _construct_questions(row)
      examples += """
    ...
    for _, row in example_data.iterrows():
      ...
      answer = row[5],
      content = row[ord(row[5]) - ord('A') + 1])
  else:
    examples = ""

  prompt = header + examples + _construct_questions(data) + "Correct answer: "
...
def load_mmlu_dataset_csv(
  ...
  for cvs_file in os.listdir(dataset_path):
      if cvs_file.endswith(".csv"):
          ...
          dataset.append(data.iloc[num_shots:])
          examples.append(data.head(num_shots))
  if not dataset:
    return None
  
  mmlu_data = []
  combined_dataset = pandas.concat(dataset, ignore_index=True)
  example_dataset = pandas.concat(examples, ignore_index=True)
  num_rows, _  = combined_dataset.shape
  print(f"Loaded {num_rows} prompts for mmlu dataset")
  for _, row in combined_dataset.iterrows():
    prompt = gen_mmlu_prompt(
      'HELM',
      row,
      example_dataset[example_dataset['subject'] == row['subject']])
    output = row[5]
...
if __name__ == "__main__":
  gc.disable()
  main(parse_args())
  #print(load_mmlu_dataset_csv('/home/beinuoz/data/data/mmlu/test'))