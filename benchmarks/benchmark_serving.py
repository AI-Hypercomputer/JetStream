...
def sample_requests(
  ...
  tokenized_dataset = tokenize_dataset(sampled_dataset, tokenizer, use_chat_template)
...
async def send_request(
  ...
  if use_chat_template:
    token_ids = tokenizer.apply_chat_template([{"role": "user", "content": input_request.prompt}], 
                            add_generation_prompt=True)
  else:
...
def parse_args() -> argparse.Namespace:
  ...
  parser.add_argument(
    ...
    default=False,
    help=(
      "Whether the tokenizer should be applying a chat template (used for instruction-tuned models)."
    )
  )
  parser.add_argument(
      "--num-prompts",
