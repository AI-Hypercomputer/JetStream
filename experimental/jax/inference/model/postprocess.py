...
def postprocess(
    ...
    if not has_prefill and has_generate:
        ...
        output.generate_next_pos = jnp.where(output.generate_done, -1, attn_meta.generate_pos + 1)
        output.generate_next_pos = jnp.where(output.generate_next_pos, output.generate_next_pos, -1)
