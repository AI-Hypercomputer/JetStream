...
def merge_gate_up_proj_weights(gate, up, num_devices):
    ...
    weight = jnp.concatenate([gate[:, :col_chunk_size], up[:, :col_chunk_size]], axis=-1)
...
def convert_hf_llama(
    ...
    jax_weight_state = {
        "embed_tokens": {
            "weight": convert_to_jax_array(torch_weight_state["model.embed_tokens.weight"]),
        },
        "layers": {},
        "norm": {
            "weight": convert_to_jax_array(torch_weight_state["model.norm.weight"]),
        },
        "lm_head": {
            "weight": convert_to_jax_array(torch_weight_state["lm_head.weight"].T),
        }
    }
    ...
    for i in range(config.num_hidden_layers):
        gate_up_proj_weight = merge_gate_up_proj_weights(
            gate=convert_to_jax_array(torch_weight_state[f"model.layers.{i}.mlp.gate_proj.weight"].T),
            up=convert_to_jax_array(torch_weight_state[f"model.layers.{i}.mlp.up_proj.weight"].T),
        ...
        jax_weight_state["layers"][i] = {
            "self_attn": {
                "q_proj": {
                    "weight": convert_to_jax_array(torch_weight_state[f"model.layers.{i}.self_attn.q_proj.weight"].T),
                },
                "k_proj": {
                    "weight": convert_to_jax_array(torch_weight_state[f"model.layers.{i}.self_attn.k_proj.weight"].T),
                },
                "v_proj": {
                    "weight": convert_to_jax_array(torch_weight_state[f"model.layers.{i}.self_attn.v_proj.weight"].T),
                },
                "o_proj": {
                    "weight": convert_to_jax_array(torch_weight_state[f"model.layers.{i}.self_attn.o_proj.weight"].T),
                },
            },
            "ffw": {
                "gate_up_proj": {
                ...
                "down_proj": {
                    "weight": convert_to_jax_array(torch_weight_state[f"model.layers.{i}.mlp.down_proj.weight"].T),
                },
            },
            "input_layernorm": {
                "weight": convert_to_jax_array(torch_weight_state[f"model.layers.{i}.input_layernorm.weight"]),
            },
            "post_attention_layernorm": {
                "weight": convert_to_jax_array(torch_weight_state[f"model.layers.{i}.post_attention_layernorm.weight"]),
            },
        }
