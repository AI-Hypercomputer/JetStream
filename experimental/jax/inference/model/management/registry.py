...
from transformers import (logging as hf_logging, AutoModelForCausalLM,
...
class ModelRegistry():
    ...
    def load_tokenizer(
        ...
        if path:
            raise ValueError(f"Load tokenizer from given path {path} is not supported")
    ...
    def load_weights_to_host(
        ...
        if source == ModelSource.HUGGINGFACE:
            if model_id not in hf_ckpt_conversion:
                raise ValueError(f"No weight conversion function for HF model {model_id}")
            ...
            if not path:
                ...
                raise NotImplemented("Loading from path for HF model is not supported yet")
