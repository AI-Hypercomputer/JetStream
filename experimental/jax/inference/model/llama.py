...
class LlamaFeedForward(nn.Module):
    def __init__(
        ...
        if parallel_config.enable_collective_matmul:
            gate_up_proj_parallel.collective_matmul_type = parallel.CollectiveMatmulType.ALL_GATHER
    ...
    def __call__(
        ...
        if self.parallel_config.gather_input:
            hidden_states = parallel.ops.all_gather(
                hidden_states, len(hidden_states.shape)-1,
...
class LlamaDecoderLayer(nn.Module):
    def __init__(
        ...
        self.ffw = LlamaFeedForward(
            ...
            parallel_config=parallel.FeedForwardParallelConfig(
                mesh,
                enable_collective_matmul=enable_collective_matmul
            )
        )
...
class LlamaModel(nn.Model):
    def __init__(
        ...
        if enable_collective_matmul:
            lm_head_parallel.collective_matmul_type = parallel.CollectiveMatmulType.ALL_GATHER
...
class LlamaForCausalLM(nn.CausalLM):
    ...
    def __call__(
        ...
        tokens, done = self.sampler.sample(logits, positions, attn_metadata, sampling_params)
