...
class Sampler:
    ...
    def sample(
        ...
        probabilities = jax.nn.softmax(logits / sampling_params.temperature, axis=-1)
        top_k_prob, top_k_indices= jax.lax.top_k(probabilities, 1)
        selected_index = jax.random.categorical(sampling_rng, top_k_prob)

        tokens = top_k_indices[jnp.arange(0, top_k_indices.shape[0]), selected_index]
        done = jnp.equal(tokens, self.eos)
        done = jnp.logical_or(done, jnp.greater_equal(positions, self.max_length-1))
