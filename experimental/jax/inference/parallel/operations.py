...
def reduce_scatter(operand, scatter_dimension, axis_names):
    ...
    half_accum_shape = operand.shape[:scatter_dimension] + (half_chunk_size,)  + operand.shape[scatter_dimension + 1 :]
    ...
    def step(i, carry):
        ...
        p_fwd_res = jax.lax.dynamic_slice_in_dim(operand, fwd_idx, half_chunk_size, scatter_dimension)
        p_bwd_res = jax.lax.dynamic_slice_in_dim(operand, bwd_idx, half_chunk_size, scatter_dimension)
    ...
    p_fwd_res = jax.lax.dynamic_slice_in_dim(operand, initial_fwd_idx, half_chunk_size, scatter_dimension)
    p_bwd_res = jax.lax.dynamic_slice_in_dim(operand, initial_bwd_idx, half_chunk_size, scatter_dimension)

    accum_fwd, accum_bwd, p_fwd_res, p_bwd_res = jax.lax.fori_loop(
        1,
        num_partitions,
        step,
        (accum_fwd, accum_bwd, p_fwd_res, p_bwd_res)
    )

    return jnp.concatenate((p_fwd_res, p_bwd_res), scatter_dimension) + jnp.concatenate((accum_fwd, accum_bwd), scatter_dimension)
...
def all_gather(operand, axis, axis_names):
    ...
    return jax.lax.all_gather(operand, axis=axis, axis_name=axis_names, tiled=True)
