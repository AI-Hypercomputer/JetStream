...
def vanilla_prefill_mha(q, k, v):
    ...
    head_dim = q.shape[2]
    causal_mask = jnp.tril(jnp.ones(shape=(total_len, total_len))).astype(jnp.bool)
...
def vanilla_prefill_gqa(q, k, v):
    ...
    q = q.reshape((total_len, num_kv_heads, num_attn_heads // num_kv_heads, head_dim))
    causal_mask = jnp.tril(jnp.ones(shape=(total_len, total_len))).astype(jnp.bool)
...
def chunked_prefill_attention(q, cache_k, cache_v, length, page_indices, accelerator="tpu"):
    if accelerator == "tpu":
