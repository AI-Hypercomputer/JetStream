...
def prepare_rhs_for_all_gather_collective_matmul(rhs: jax.Array, mesh: Mesh):
    ...
    def reshuffle(rhs: jax.Array):
        ...
        def swap(i, carry):
            ...
            operand_1 = jax.lax.dynamic_slice_in_dim(rhs, idx_1, half_rhs_chunk_row_size, axis=0)
            operand_2 = jax.lax.dynamic_slice_in_dim(rhs, idx_2, half_rhs_chunk_row_size, axis=0)
...
def all_gather_collective_matmul(lhs, rhs, axis_names):
    ...
    accum, fwd_lhs, bwd_lhs = jax.lax.fori_loop(0, num_partitions - 1, step, (accum, fwd_lhs, bwd_lhs))

    # Last round which doesn't need collective permute.
    rhs_row_idx = ((idx + num_partitions - 1) % num_partitions) * rhs_chunk_row_size
    cur_lhs = jnp.concatenate((fwd_lhs, bwd_lhs), axis=1)
    rhs_chunk = jax.lax.dynamic_slice_in_dim(rhs, rhs_row_idx, rhs_chunk_row_size)
    output =  cur_lhs @ rhs_chunk
    accum += output
...
def prepare_rhs_for_collective_matmul_reduce_scatter(rhs: jax.Array, mesh: Mesh):
    ...
    def reshuffle(rhs):
        ...
        def swap(i, carry):
            ...
            operand_1 = jax.lax.dynamic_slice_in_dim(rhs, idx_1, half_rhs_chunk_col_size, axis=1)
            operand_2 = jax.lax.dynamic_slice_in_dim(rhs, idx_2, half_rhs_chunk_col_size, axis=1)
...
def collective_matmul_reduce_scatter(lhs, rhs, axis_names):
    ...
    fwd_accum, bwd_accum, partial_res = jax.lax.fori_loop(
        ...
        num_partitions,
        step,
        (fwd_accum, bwd_accum, partial_res)
    )
    accum = jnp.concatenate((fwd_accum, bwd_accum), axis=1)
    accum += partial_res
    return accum
