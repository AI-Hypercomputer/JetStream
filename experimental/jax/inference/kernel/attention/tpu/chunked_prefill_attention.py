    ...
    def per_kv_chunk_body(i, _):
        ...
        def body():
            ...
            def per_group_body(group_idx, _):
                ...
                out_ref[group_idx, :, :] = (out * jax.lax.broadcast_in_dim(jnp.exp(prev_m - cur_m), (chunk_size, head_dim), (0, 1)) + p @ v).astype(out_ref.dtype)   # p @ v  "ts,sd->td"
        ...
        def rescale():
            l = jax.lax.broadcast_in_dim(l_ref[...], (group_size, chunk_size, head_dim), (0, 1, 2))
...
def chunked_prefill_attention(
    ...
    out, _, _  = pl.pallas_call(
        ...
        # interpret=True,
        # debug=True
    )(jnp.reshape(length, (1,)), page_indices, jnp.asarray([0], jnp.int32), q, k_pages, v_pages)
