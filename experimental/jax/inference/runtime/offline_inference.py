...
class OfflineInference:
    def __init__(
        ...
        self._running_pool[mp.Process | threading.Thread] = []
    ...
    def launch_engine(self, req_queue, res_queue, model_load_params, started_event, completion_event):
        devices = jax.devices()
    ...
    def __call__(self, prompts: Sequence[str]) -> list[Response]:
        for i in range(self.num_engines):
            while not self._engine_started_events[i].is_set():
                self._engine_started_events[i].wait()

        print("all the engines started:", datetime.datetime.now(), "processing requests...")
        ...
        for i in range(self.num_engines):
            ...
            for p in prompts_slice:
                self.req_queues[i].put(
                    OfflineRequest(prompt=p)
                )
