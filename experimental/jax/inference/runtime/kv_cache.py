...
class KVCacheStorage:
    ...
    def init_hbm_storage(
        ...
        if memory_stats: 
            ...
            available_hbm_bytes = memory_stats['bytes_reservable_limit'] * hbm_utilization - memory_stats['bytes_in_use']
            item_size = jnp.ones((1), dtype=cache_dtype).itemsize
            kv_size_per_page =  item_size * self.num_kv_heads // self.num_devices * self.head_dim * self.page_size * 2 
            num_pages_per_layer = int(available_hbm_bytes // kv_size_per_page // self.num_layers)
        else:
        ...
        kv_storage = [KVCache(
            k=jnp.ones(
                shape=(self.num_kv_heads, num_pages_per_layer, self.page_size, self.head_dim),
                device=NamedSharding(self.mesh, P(self.mesh.axis_names, None, None, None)),
                dtype=cache_dtype,
            ),
            v=jnp.ones(
                shape=(self.num_kv_heads, num_pages_per_layer, self.page_size, self.head_dim),
