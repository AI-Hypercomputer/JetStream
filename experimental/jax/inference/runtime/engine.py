...
class InferenceParams:
    ...
    prefill_chunk_sizes: list[int] = dataclasses.field(default_factory=lambda: [128, 256, 512, 1024])
...
class Engine:
    def __init__(
        ...
        model_config = model_registry.load_model_config(
        ...
        self.kv_manager = KVCacheManager(num_hbm_pages, self.inference_params.page_size)
        ...
        self.generate_state = GenerateState(
            token_ids=jnp.zeros(
                shape=(self.inference_params.max_num_seqs,), dtype=jnp.int32,
                device=NamedSharding(self.mesh, P(None))
            ),
            positions=jnp.full(
                shape=(self.inference_params.max_num_seqs,), fill_value=-1,
                dtype=jnp.int32, device=NamedSharding(self.mesh, P(None))),
            page_table=jnp.full(
                shape=(self.inference_params.max_num_seqs, self.inference_params.max_seq_length//self.inference_params.page_size), 
                fill_value=self.kv_manager.dummy_page_idx, 
                dtype=jnp.int32, device=NamedSharding(self.mesh, P(None, None))
            ),
            available_slots=slots,
            active_slot_req_map={},
        )
        self.sample_params = SamplingParams(
            jax.device_put(jnp.asarray((1.0),dtype=jnp.float32), NamedSharding(self.mesh, P())),
            jax.device_put(jnp.asarray((1), dtype=jnp.int32), NamedSharding(self.mesh, P())),
            jax.device_put(jax.random.key(0), NamedSharding(self.mesh, P())),
        )

        self.num_pages_per_seq = self.inference_params.max_seq_length // self.inference_params.page_size
        ...
        self._max_device_requests_sem = threading.Semaphore(self.inference_params.max_num_seqs * 1.5)
    ...
    def _preprocess(self) -> jax.Array:
        while True:
            ...
            if num_paddings < 0:
                ...
                req.prompt_token_ids = token_id_list[-self.inference_params.max_input_length:]
            else:
                padded_tokens = np.pad(tokens, (0, self.inference_params.max_input_length-token_len))
            ...
            dummy_page_indices = [self.kv_manager.dummy_page_idx for _ in range(self.num_pages_per_seq)]

            # Select chunk size.
            # TODO: move it to a function.
            chunk_size_idx = 0
            chunk_sizes = self.inference_params.prefill_chunk_sizes
            while chunk_size_idx < len(chunk_sizes) and token_len > chunk_sizes[chunk_size_idx]:
                chunk_size_idx += 1
            chunk_size_idx = chunk_size_idx - 1 if chunk_size_idx == len(chunk_sizes) else chunk_size_idx
            ...
            self.scheduler.enqueue_prefill_req(
                PrefillRequest(
                    ...
                    chunk_idx=0,
                    chunk_size=self.inference_params.prefill_chunk_sizes[chunk_size_idx],
        ...
        while True:
            ...
            if schedule.schedule_generate:
                ...
                with self.generate_state.map_mutex:
                    for slot, processed_gr in self.generate_state.active_slot_req_map.items():
                        processed_gr.pos += 1
    ...
    def _postprocess(self) -> str:
        while True:
                ...
                with self.generate_state.map_mutex:
                    ...
                    for slot in self.generate_state.active_slot_req_map.keys():
                        if p_req.generate_done[slot]:
                            ...
                            pages_to_free = self.generate_state.active_slot_req_map[slot].page_indices
            ...
            if p_req.prefill_request_id:
                ...
                generated_text = self.tokenizer._convert_id_to_token(p_req.prefill_token_id).replace("▁", " ")
            ...
            for slot, req_id in zip(p_req.generate_active_slots, p_req.generate_active_request_ids):
                ...
                generated_text = self.tokenizer._convert_id_to_token(p_req.generate_token_ids[slot]).replace("▁", " ")
                req.generated_text += generated_text
