...
class Executor:
  ...
  def __init__(
    ...
    self.jitted_prepare_model_input_func = jax.jit(
        self.prepare_model_input, static_argnames=("chunk_size",)
    )
  ...
  def compile(
    ...
    for prefill_chunk_size in prefill_chunk_sizes:
      ...
      dummy_prefill_page_update_indices = [
          0 for _ in range(prefill_chunk_size // page_size)
      ]
  ...
  def prepare_input_and_update_generate_state(
    ...
    input_ids, generate_tokens, positions, attn_meta = (
        self.jitted_prepare_model_input_func(
            attn_meta,
            prefill_tpp,
            prefill_cur_length,
            chunk_id,
