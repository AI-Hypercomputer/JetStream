...
ModelForwardFunc = Callable[
    [dict, jax.Array, jax.Array, list[KVCache], AttentionMetadata, SamplingParams],
    tuple[ModelOutput, list[KVCache]]]

Executable = ModelForwardFunc
...
class Executor:
    def __init__(
        ...
        self.executables_dict: dict[str: Executable] = {}
        ...
        self.jitted_prepare_model_input_func = jax.jit(self.prepare_model_input, static_argnames=("chunk_size",))
    ...
    def compile(
        ...
        if self.debug_mode:
            print("No compilation under debug mode")

        page_size = kv_caches[0].k.shape[2]

        # Prefill only compile.
        for size in prefill_chunk_sizes:
            dummy_padded_tensor = jnp.ones((max_input_len,), dtype=jnp.int32, device=NamedSharding(self.mesh, P(None)))
            dummy_page_indices_tensor = jnp.ones(
                (max_seq_len//page_size,), dtype=jnp.int32, device=NamedSharding(self.mesh, P(None)))
            dummy_page_indices = np.asarray(dummy_page_indices_tensor).tolist()
            dummy_page_update_indices = [0 for _ in range(size // page_size)]
            ...
            dummy_schedule = Schedule(
                ...
                prefill_pages_update=PrefillPagesUpdate(page_indices=dummy_page_update_indices),
        ...
        dummy_batch_tensor = jnp.ones((max_num_seqs), dtype=jnp.int32, device=NamedSharding(self.mesh, P(None)))
        dummy_page_table_tensor = jnp.ones(
            (max_num_seqs, max_seq_len//page_size), dtype=jnp.int32,
        ...
        for prefill_chunk_size in prefill_chunk_sizes:
            dummy_padded_prompt_tensor = jnp.ones((max_input_len,), dtype=jnp.int32, device=NamedSharding(self.mesh, P(None)))
            dummy_prefill_page_indices = np.ones((max_seq_len//page_size,), dtype=np.int32,).tolist()
            dummy_prefill_page_update_indices = [0 for _ in range(prefill_chunk_size // page_size)]
            ...
            dummy_schedule = Schedule(
                ...
                prefill_pages_update=PrefillPagesUpdate(page_indices=dummy_prefill_page_update_indices),
                schedule_generate=True,
                new_generate_requests=[],
                generate_state_page_updates=[],
            )
    ...
    def compile_once(
        ...
        print(f"Compilation for {key} completed, take {(end_time-start_time).total_seconds()} seconds")

    def execute(self, input: ModelForwardInput) -> tuple[ModelOutput, list[KVCache]]:
        key = self.executable_key(input.attn_metadata)
        if self.debug_mode:
            return self.model_forward(input)

        if key not in self.executables_dict:
            print(("Warning: the cache is missing, "
    ...
    def executable_key(self, attn_meta: AttentionMetadata) -> str:
        ...
        if len(attn_meta.prefill_pos.shape) > 0:
            prefill_chunk_size = attn_meta.prefill_pos.shape[0]
    ...
    def jitted_model_forward_func(self, input: ModelForwardInput):
        return jax.jit(
                self.shard_mapped_model_forward_func(input),
                donate_argnums=(3,)
            )

    def shard_mapped_model_forward_func(self, input: ModelForwardInput):
        return shard_map(
    ...
    def prepare_input_and_update_generate_state(
        ...
        update_generate_tpp = (self.dummy_scalar, self.dummy_scalar, self.dummy_scalar)
        ...
        if schedule.schedule_prefill:
            pr = schedule.prefill_request
            prefill_tpp = (pr.device_token_ids, pr.device_positions, np.array(pr.page_indices))
            prefill_cur_length = (pr.chunk_idx + 1) * pr.chunk_size
            prefill_total_len = len(pr.unpadded_token_ids)
            if prefill_cur_length > prefill_total_len:
                prefill_cur_length = prefill_total_len

            chunk_id = pr.chunk_idx
            chunk_size = pr.chunk_size

        if schedule.schedule_generate:
            generate_tpp = (generate_state.token_ids, generate_state.positions, generate_state.page_table)
            update_token_ids = []
            update_pos = np.full((max_num_seqs,), 1e6, dtype=np.int32)
            update_page_indices = np.full((max_num_seqs, self.num_pages_per_seq), 1e6, dtype=np.int32)
        ...
        input_ids, generate_tokens, positions, attn_meta = self.jitted_prepare_model_input_func(
            attn_meta,
            prefill_tpp,
            prefill_cur_length,
            chunk_id,
    ...
    def prepare_model_input(
        ...
        if len(g_tokens.shape) > 0:
            update_g_tokens, update_g_positions, update_g_page_table = update_generate_tpp
            ...
            g_page_table = g_page_table.at[generate_pt_update_slots, generate_pt_update_page_idxs].set(generate_page_updates)
        ...
        if len(p_tokens.shape) > 0 and len(g_tokens.shape) > 0:
            ...
            raise ValueError("Failed to build the input as no prefill or generate gets scheduled")
