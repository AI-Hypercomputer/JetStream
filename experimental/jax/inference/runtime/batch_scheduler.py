    ...
    def schedule(
        ...
        if next_prefill_req:
            ...
            if total_len <= (next_prefill_req.chunk_idx + 1) * next_prefill_req.chunk_size:
                cur_prompt_chunk_len = total_len - next_prefill_req.chunk_idx * next_prefill_req.chunk_size
            alloced_pages = self.kv_manager.alloc_prefill_hbm_pages(cur_prompt_chunk_len)
            if len(alloced_pages) == 0:
                ...
                start_idx = (next_prefill_req.chunk_idx * next_prefill_req.chunk_size) // self.kv_manager.page_size
        ...
        with generate_state.map_mutex:
            if (
                ...
                or (len(generate_state.active_slot_req_map) + self.generate_queue.qsize() > 0.95 * self.max_num_seqs)
            ):
                # Add new generate request to the slots.
                while generate_state.available_slots.qsize() > 0 and self.generate_queue.qsize() > 0:
                ...
                alloced_pages = self.kv_manager.alloc_hbm_pages(len(generate_state.active_slot_req_map))
                if len(generate_state.active_slot_req_map) != 0 and len(alloced_pages) == 0:
                    raise NotImplementedError("Eviction isn't supported yet, please set a lower value for max_num_seqs")
