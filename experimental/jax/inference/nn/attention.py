...
class AttentionMetadata:
    prefill_length: jax.Array | NamedSharding  # shape: []; Prefill True length without padding
    ...
    generate_page_table: jax.Array | NamedSharding # shape: [generate_batch, max_len // page_size]
...
class AttentionOps(Module):
    ...
    def _write_prefill_kv_to_kv_cache(self, k, v, kv_cache: KVCache, unpadded_len, page_table):
        ...
        k = k.reshape((num_kv_heads_per_device, -1, page_size, self.head_dim)).astype(kv_cache.k.dtype)
        v = v.reshape((num_kv_heads_per_device, -1, page_size, self.head_dim)).astype(kv_cache.v.dtype)
        ...
        idx = 0
        kv_cache, idx = jax.lax.while_loop(update_cond, per_page_update, (kv_cache, idx))
    ...
    def _write_generate_kv_to_kv_cache(self, k, v, kv_cache, pos, page_table):
        ...
        kv_heads_axis_stride = jnp.repeat(jnp.arange(0, num_kv_heads_per_device), num_tokens) * num_pages * page_size
        ...
        kv_cache.k = kv_cache.k.reshape(num_kv_heads_per_device, num_pages, page_size, head_dim)
        kv_cache.v = kv_cache.v.reshape(num_kv_heads_per_device, num_pages, page_size, head_dim)

        return kv_cache

    def _prefill(self, q, k, v, kv_cache: KVCache, attn_metadata: AttentionMetadata):
        ...
        output = kernel.chunked_prefill_attention(
            ...
            attn_metadata.prefill_length, attn_metadata.prefill_page_table)
...
#            output = kernel.vanilla_prefill_gqa(q, k, v, attn_metadata.prefill_length)
    ...
    def _generate(self, q, k, v, kv_cache: KVCache, attn_metadata: AttentionMetadata):
        ...
        output = kernel.decode_attention(
            q, kv_cache.k, kv_cache.v, attn_metadata.generate_pos,
            attn_metadata.generate_page_table)
    ...
    def _mixed_prefill_generate(self, q, k , v, kv_cache: KVCache, attn_metadata: AttentionMetadata):
    ...
    def __call__(self, q, k, v, kv_cache: KVCache, attn_metadata: AttentionMetadata):
