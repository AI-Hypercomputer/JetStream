...
class Linear(Module):
    def __init__(
        ...
        self._num_merged = len(out_features) if isinstance(out_features, Sequence) else 1
    ...
    def __call__(self, input):
        if self.parallel_config.collective_matmul_type != None and self._collective_matmul:
        ...
        output = jnp.matmul(input, self.weight.value, preferred_element_type=preferred_type)
        ...
        if parallel_config.reduce_output:
            ...
            output = jax.lax.psum_scatter(output, axis_names, scatter_dimension=1, tiled=True)
        ...
        for k, v in weights_dict.items():
            ...
            if isinstance(attr, Parameter):
                ...
                if v.shape != param.shape:
                    logging.warning(f"Not matched shape" +
                ...
                if self.parallel_config.collective_matmul_type == parallel.CollectiveMatmulType.ALL_GATHER:
                    param.value = kernel.prepare_rhs_for_all_gather_collective_matmul(
                        param.value, self.parallel_config.mesh
                    )
                elif self.parallel_config.collective_matmul_type == parallel.CollectiveMatmulType.REDUCE_SCATTER:
                    param.value = kernel.prepare_rhs_for_collective_matmul_reduce_scatter(
                        param.value, self.parallel_config.mesh
                    )
                res[k] = param.value
            else:
                logging.warning(f"Unknown checkpoint key {k} for module {self.__class__}")
