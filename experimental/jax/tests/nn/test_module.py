...
class ModuleTest(absltest.TestCase):
    def test_random_code_initialize(self):
        w0, w1, w2, w3 = jnp.ones((1,)), jnp.ones((2,)), jnp.ones((3,)), jnp.ones((4,))
        ...
        np.testing.assert_raises(
            AssertionError, np.testing.assert_array_equal, w0, parent_module.w0.value
        )
        np.testing.assert_raises(
            AssertionError, np.testing.assert_array_equal, w1, h1_child_0_module.w1.value
        )
        np.testing.assert_raises(
            AssertionError, np.testing.assert_array_equal, w2, h1_child_1_module.w2.value
        )
        np.testing.assert_raises(
            AssertionError, np.testing.assert_array_equal, w3, h2_child_0_module.w3.value
        )
