...
class NormTest(absltest.TestCase):
    ...
    def test_rmsnorm_per_device_forward(self):
        ...
        dis_weight = distributed_rmsnorm_layer.load_weights_dict({"weight": sharded_weight})
        expect = rmsnorm_layer(input)

        sharded_input = jax.device_put(input, NamedSharding(mesh, P(None, parallel.tp_axis_names())))
        ...
        got = jax.experimental.shard_map.shard_map(
            distributed_rms_ag,
            mesh,
            in_specs=(P(parallel.tp_axis_names()), P(None, parallel.tp_axis_names())),
