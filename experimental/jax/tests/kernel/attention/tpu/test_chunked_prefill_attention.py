...
class ChunkedPrefillTest(absltest.TestCase):
    def test(self):
        ...
        mesh = jax.sharding.Mesh(
            np.reshape(devices[0], (1,)), ("a",)
        )
        rng = jax.random.key(0)

        attn_layer = nn.AttentionOps(num_attn_heads, num_kv_heads, head_dim, mesh)
        k_hbm, v_hbm = (
        ...
        page_table = jnp.array(
            ([i for i in range(num_pages_with_padding)])
        )
        ...
        num_active_pages_per_prefill = math.ceil(prefill_non_padding_len / page_size)
        ...
        for i in range(compute_times):
            ...
            if i < compute_times - 1:
                np.testing.assert_allclose(np.array(chunk_output), np.array(expected_output[idx:idx+chunk_size]), rtol=4e-03, atol=1e-03)
            if i == compute_times - 1:
                offset = prefill_non_padding_len % chunk_size
                np.testing.assert_allclose(np.array(chunk_output[:offset]), np.array(expected_output[idx:idx+offset]), rtol=4e-03, atol=1e-03)
