...
class AdapterTensorStore:
  ...
  def _unsafe_transfer_to_hbm(self, adapter_id: str):
    ...
    while (self.current_hbm_usage + metadata.size_hbm) > self.hbm_memory_budget:
      if not self._evict(from_hbm=True):
        raise RuntimeError("Not enough HBM to transfer adapter, and HBM eviction failed.")
  ...
  def _unsafe_transfer_to_cpu(self, adapter_id: str):
    ...
    while (self.current_cpu_usage + metadata.size_cpu) > self.cpu_memory_budget:
      if not self._evict(from_hbm=False):
        raise RuntimeError("Not enough CPU RAM to transfer adapter, and CPU eviction failed.")
  ...
  async def load_adapter(
    ...
    async with self.lock:
      ...
      if metadata.status == AdapterStatus.UNLOADED:   # Check if it was UNLOADED
        ...
        metadata.loading_event = asyncio.Event() # Create event *before* releasing lock
    ...
    try:
      ...
      async with self.lock:
        ...
        if (not load_successful
            and self.adapter_registry[adapter_id].status == AdapterStatus.LOADING):
          metadata.status = AdapterStatus.UNLOADED    # Mark as unloaded on error


  async def get_lora_config(self, adapter_id: str, load_if_not_loaded: bool = False):
  ...
  async def get_lora_weights(self,
      adapter_id,
      to_hbm: bool = True,
    ...
    if metadata.status not in (AdapterStatus.LOADED_HBM, AdapterStatus.LOADED_CPU):
  ...
  def _evict(self, from_hbm: bool = True) -> bool:
    ...
    if from_hbm:
      ...
      self._unsafe_unload_adapter(lru_adapter_id)   # This is not synchronous, but ONLY within the lock
    return True
