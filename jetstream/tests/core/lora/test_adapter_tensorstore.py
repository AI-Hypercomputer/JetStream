...
def create_mock_weights(size_multiplier: int = 1, dtype=np.float32) -> Dict[str, np.ndarray]:
...
class AdapterTensorStoreTest(parameterized.TestCase, unittest.IsolatedAsyncioTestCase): # Use IsolatedAsyncioTestCase
  ...
  async def asyncSetUp(self):
    ...
    self.mock_size_hbm_a = _get_size_of_pytree(_as_jnp_array(self.mock_weights_a))
    ...
    await self.store.register_adapter("adapter_a", adapter_config=self.mock_config_a)
    await self.store.register_adapter("adapter_b", adapter_config=self.mock_config_b)
  ...
  async def test_register_adapter_without_config_triggers_load(self):
    ...
    self.mock_engine.load_single_adapter.side_effect = lambda p: (mock_weights_new, mock_config_new) if p == adapter_path else FileNotFoundError
    ...
    self.assertEqual(metadata.adapter_id, adapter_id)
    self.assertEqual(metadata.adapter_path, adapter_path)
    self.assertEqual(metadata.config, mock_config_new) # Config populated by load
    ...
    self.assertEqual(metadata.status, AdapterStatus.LOADED_HBM) # Default load is HBM
  ...
  async def test_register_adapter_load_fails_no_config(self):
    ...
    with self.assertRaisesRegex(ValueError, f"Failed to read adapter_config from {adapter_path}"):
  ...
  async def test_register_adapter_duplicate_logs_warning(self):
    ...
    with self.assertLogs(level='WARNING') as log:
      await self.store.register_adapter(adapter_id, adapter_config={'rank': 32}) # Duplicate

    self.assertIn(f"Adapter with ID '{adapter_id}' already registered.", log.output[0])
    # Ensure registry wasn't overwritten
    self.assertIs(self.store.adapter_registry[adapter_id], initial_metadata)
    self.assertEqual(self.store.adapter_registry[adapter_id].config, self.mock_config_a)
    self.mock_engine.load_single_adapter.assert_not_called()
  ...
  async def test_load_unregistered_adapter_raises_error(self):
    with self.assertRaisesRegex(ValueError, "Adapter with ID 'unregistered' not registered."):
  ...
  async def test_load_new_adapter_success(self, to_hbm, final_status, convert_func_name):
    ...
    with patch(f'jetstream.core.lora.adapter_tensorstore.{convert_func_name}',
        wraps=getattr(adapter_tensorstore, convert_func_name)) as mock_convert:
    ...
    self.mock_engine.load_single_adapter.assert_called_once_with(f"{self.adapters_dir_path}/{adapter_id}")
  ...
  async def test_load_adapter_with_preloaded_weights_success(self):
    ...
    await self.store.load_adapter(adapter_id, adapter_weights=weights_np, to_hbm=True)
  ...
  async def test_load_adapter_already_loaded_correct_location(self, to_hbm, initial_status):
    ...
    if initial_status == AdapterStatus.LOADED_HBM:
      self.store.loaded_adapters_hbm[adapter_id] = _as_jnp_array(self.mock_weights_a)
    ...
    with patch.object(self.store, '_unsafe_transfer_to_hbm') as mock_t_hbm, \
         patch.object(self.store, '_unsafe_transfer_to_cpu') as mock_t_cpu:
      await self.store.load_adapter(adapter_id, to_hbm=to_hbm)
  ...
  @parameterized.named_parameters(
    ('cpu_to_hbm', False, True, AdapterStatus.LOADED_HBM, '_unsafe_transfer_to_hbm'),
    ('hbm_to_cpu', True, False, AdapterStatus.LOADED_CPU, '_unsafe_transfer_to_cpu'),
  )
  async def test_load_adapter_triggers_transfer(self, initial_hbm, to_hbm, final_status, transfer_method_name):
    ...
    with patch.object(self.store, transfer_method_name, wraps=getattr(self.store, transfer_method_name)) as mock_transfer:
      await self.store.load_adapter(adapter_id, to_hbm=to_hbm)
  ...
  @patch('asyncio.get_running_loop') # Need to mock the loop for run_in_executor
  async def test_load_adapter_waits_for_loading_state(self, mock_get_loop):
    ...
    # 'await load_finished_event.wait()'
    task1 = asyncio.create_task(self.store.load_adapter(adapter_id, to_hbm=True))
    ...
    async with self.store.lock:
      self.assertEqual(self.store.adapter_registry[adapter_id].status, AdapterStatus.LOADING)
    ...
    task2 = asyncio.create_task(self.store.load_adapter(adapter_id, to_hbm=True))
  ...
  async def test_load_single_adapter_returning_none(self):
    ...
    self.mock_engine.load_single_adapter.side_effect = \
        lambda p: (None, None) \
        if p == adapter_path else FileNotFoundError

    with self.assertRaisesRegex(ValueError,
        f"Failed to load adapter_weights from {adapter_path}."):
  ...
  async def test_load_adapter_with_changed_status_before_loading(self):
    ...
    self.assertIn(expected_log, cm.output[0]) # Check the first (and only) logged line
  ...
  async def test_load_triggers_hbm_eviction(self):
    ...
    self.store.hbm_memory_budget = self.mock_size_hbm_a + self.mock_size_hbm_b + self.mock_size_hbm_c // 2 # Fits A & B, but not A+B+C
    ...
    with patch.object(self.store, '_evict', wraps=self.store._evict) as mock_evict, \
         patch.object(self.store, '_unsafe_transfer_to_cpu', wraps=self.store._unsafe_transfer_to_cpu) as mock_transfer_cpu:

      await self.store.load_adapter("adapter_c", to_hbm=True) # Load C, should evict B
    ...
    # Verify final state
    self.assertEqual(self.store.adapter_registry["adapter_a"].status, AdapterStatus.LOADED_CPU) # A remains
    self.assertEqual(self.store.adapter_registry["adapter_b"].status, AdapterStatus.LOADED_HBM) # B transferred
    self.assertEqual(self.store.adapter_registry["adapter_c"].status, AdapterStatus.LOADED_HBM) # C loaded
    ...
    self.assertEqual(self.store.current_hbm_usage, self.mock_size_hbm_b + self.mock_size_hbm_c)
    self.assertEqual(self.store.current_cpu_usage, self.mock_size_cpu_a)
  ...
  async def test_load_triggers_cpu_eviction(self):
    ...
    self.store.cpu_memory_budget = self.mock_size_hbm_a + self.mock_size_hbm_b + self.mock_size_hbm_c // 2 # Fits A & B, but not A+B+C
    ...
    with patch.object(self.store, '_evict', wraps=self.store._evict) as mock_evict, \
         patch.object(self.store, '_unsafe_unload_adapter', wraps=self.store._unsafe_unload_adapter) as mock_unload:

      await self.store.load_adapter("adapter_c", to_hbm=False) # Load C to CPU, should evict A
    ...
    # Verify final state
    self.assertEqual(self.store.adapter_registry["adapter_a"].status, AdapterStatus.UNLOADED) # A unloaded
    self.assertEqual(self.store.adapter_registry["adapter_b"].status, AdapterStatus.LOADED_CPU) # B remains
    self.assertEqual(self.store.adapter_registry["adapter_c"].status, AdapterStatus.LOADED_CPU) # C loaded
    ...
    self.assertEqual(self.store.current_cpu_usage, self.mock_size_cpu_b + self.mock_size_cpu_c)
    self.assertEqual(self.store.current_hbm_usage, 0)
  ...
  async def test_load_hbm_eviction_fails_raises_error(self):
    ...
    with patch.object(self.store, '_evict', return_value=False) as mock_evict:
      with self.assertRaisesRegex(RuntimeError, "Not enough HBM to load adapter, and eviction failed."):
    ...
    self.assertEqual(self.store.adapter_registry["adapter_b"].status, AdapterStatus.UNLOADED)
    self.assertEqual(self.store.adapter_registry["adapter_a"].status, AdapterStatus.LOADED_HBM) # A remains
    self.assertEqual(self.store.current_hbm_usage, self.mock_size_hbm_a)
    self.assertEqual(self.store.running_requests, 0)
  ...
  async def test_load_cpu_eviction_fails_raises_error(self):
    ...
    # Mock _evict to simulate no adapter can be evicted
    with patch.object(self.store, '_evict', return_value=False) as mock_evict:
      with self.assertRaisesRegex(RuntimeError, "Not enough CPU RAM to load adapter, and eviction failed."):
  ...
  async def test_load_fails_during_io(self):
    ...
    await self.store.register_adapter(adapter_id, adapter_config=get_mock_config()) # Register first
    self.mock_engine.load_single_adapter.reset_mock()
  ...
  async def test_unload_adapter_success(self, loaded_hbm):
    ...
    adapter_id = "adapter_a"
    await self.store.load_adapter(adapter_id, to_hbm=loaded_hbm) # Load it first
    self.assertTrue(self.store.adapter_registry[adapter_id].status in
        (AdapterStatus.LOADED_HBM, AdapterStatus.LOADED_CPU))
    initial_hbm = self.store.current_hbm_usage
    initial_cpu = self.store.current_cpu_usage

    with patch.object(self.store, '_unsafe_unload_adapter', wraps=self.store._unsafe_unload_adapter) as mock_unsafe_unload:
      await self.store.unload_adapter(adapter_id)
      mock_unsafe_unload.assert_called_once_with(adapter_id)

    metadata = self.store.adapter_registry[adapter_id]
    self.assertEqual(metadata.status, AdapterStatus.UNLOADED)
    self.assertEqual(self.store.current_hbm_usage, 0 if loaded_hbm else initial_hbm)
    self.assertEqual(self.store.current_cpu_usage, 0 if not loaded_hbm else initial_cpu)
    self.assertNotIn(adapter_id, self.store.loaded_adapters_hbm)
    self.assertNotIn(adapter_id, self.store.loaded_adapters_cpu)
  ...
  async def test_unload_unregistered_adapter_raises_error(self):
    with self.assertRaisesRegex(ValueError, "Adatper with ID 'unknown' not found."):
  ...
  async def test_unload_already_unloaded_adapter_is_noop(self):
    ...
    self.assertEqual(self.store.adapter_registry[adapter_id].status, AdapterStatus.UNLOADED)

    with patch.object(self.store, '_unsafe_unload_adapter', wraps=self.store._unsafe_unload_adapter) as mock_unsafe_unload:
      await self.store.unload_adapter(adapter_id) # Should do nothing
      mock_unsafe_unload.assert_called_once_with(adapter_id) # Unsafe method called once
  ...
  async def test_unload_waits_for_loading(self):
    ...
    with patch('asyncio.get_running_loop') as mock_get_loop:
      ...
      # Start loading task
      load_task = asyncio.create_task(self.store.load_adapter(adapter_id, to_hbm=True))
      await asyncio.sleep(0.01) # Let load start
  ...
  async def test_get_lora_config_unregistered_raises_error(self):
    with self.assertRaisesRegex(ValueError, "LoRA adapter with id=unknown is not loaded."):
  ...
  async def test_get_lora_config_unregistered_with_load_flag(self):
    ...
    config = await self.store.get_lora_config(adapter_id, load_if_not_loaded=True)

    self.assertEqual(config, mock_config_new)
    self.assertIn(adapter_id, self.store.adapter_registry)
    self.mock_engine.load_single_adapter.assert_called_once_with(adapter_path)
    # Check status after the mocked synchronous load
    self.assertEqual(self.store.adapter_registry[adapter_id].status, AdapterStatus.LOADED_HBM)
  ...
  async def test_get_lora_weights_hbm_loaded(self):
    ...
    self.assertEqual(self.store.adapter_registry[adapter_id].last_accessed, start_time)
  ...
  async def test_get_lora_weights_needs_register_and_load(self):
    ...
    weights = await self.store.get_lora_weights(adapter_id, to_hbm=True, load_if_not_loaded=True)
  ...
  async def test_get_lora_weights_with_unregistered_adapter(self):
    ...
    with self.assertRaisesRegex(ValueError,
        f"LoRA adapter with id={adapter_id} is not loaded."):
      weights = await self.store.get_lora_weights(adapter_id, to_hbm=True)
  ...
  @parameterized.named_parameters(
      ('cpu_to_hbm', False, True, AdapterStatus.LOADED_HBM,
        '_unsafe_transfer_to_hbm'),
      ('hbm_to_cpu', True, False, AdapterStatus.LOADED_CPU,
        '_unsafe_transfer_to_cpu'),
  )
  async def test_get_lora_weights_needs_transfer(self, initial_hbm, to_hbm,
      final_status, transfer_method_name):
    """Test getting weights triggers transfer."""
    adapter_id = "adapter_a"
    await self.store.load_adapter(adapter_id, to_hbm=initial_hbm) # Load to CPU

    with patch.object(self.store, transfer_method_name, wraps=getattr(self.store, transfer_method_name)) as mock_transfer:
      weights = await self.store.get_lora_weights(adapter_id, to_hbm=to_hbm) # Request HBM
    ...
    self.assertEqual(self.store.adapter_registry[adapter_id].status, final_status)
  ...
  async def test_register_adapter_with_concurrent_registrations(self):
    ...
    with patch('asyncio.get_running_loop') as mock_get_loop, \
      ...
      # Start register task
      register_task = asyncio.create_task(self.store.register_adapter(adapter_id))
      await asyncio.sleep(0.01) # Let load start
